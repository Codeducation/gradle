...
It may also contain additional build logic for Gradle (custom tasks) that is not covered by commonly available Gradle plugins.
...
[NOTE]
The domain of our example is to get informed about Gradle Build Tool releases.
...
In Gradle, assigning a type is done by applying a plugin.
...
For example, if your project should be a "Java Library", it would not only apply the `java-library` plugin but also do configuration such as setting the Java version to 11.
You can do configuration like that directly in `release/build.gradle(.kts)` but than you would have to repeat these configuration in other components that also contain "Java Library" projects.
...
You can find more information about writing convention plugins in the chapter on TODO and the samples on build logic organisation TODO.
FOr adding build logic TODO (link to plugin dev chapteres).
...
- _Make components (builds) known to each other._ This is done by adding `includeBuild(...)` statements to `settings.gradle(.kts)`.
...
- _Declare dependencies between (projects of) components._
This is done similarly to declaring dependencies to binary components by using GA (_group_ and _artifact_) coordinates in the `dependencies { }` block of a `build.gradle(.kts)` file:
`implementation("com.myorg.platform:product-platform")`.
Or, if the included component provides a plugin, you apply the plugin by ID similar to how you would apply a plugin from the plugin repository:
...
1. Compact `build.gradle(.kts)` files. While traditionally... TODO
2. Isolate cross-cutting techical concerns into project types. Conventions plugins... TODO
3. Central place for declaring where components come from. Binary and source alike... TODO
4. Central platform declaration. TODO - What's a platform in one sentence
5. Isolating components: Easy to move them phyiscially. Change from source to binary. Internal structure can change. Don't let them get too large.
